...
def check_traffic_lights():
    ...
    if dropped_vehicle.is_at_traffic_light():
        ...
        #     write code for get red light time
        #     print(red_time)
        ...
        if traffic_light.get_state()==carla.TrafficLightState.Yellow:
            ...
            dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.3, steer=0.21))
            time.sleep(5)

            dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.1, steer=0.2))
            time.sleep(5)
